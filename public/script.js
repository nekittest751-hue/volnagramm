...
function chatSubtitle(chat) {
  ...
  if (!chat.is_group) {
    const peer = state.currentChatMeta?.members?.find((m) => m.id !== state.user.id);
    if (peer?.status) return peer.status;
    if (chat.peer_status) return chat.peer_status;
    return 'личный чат';
  }
  return 'группа';
}
...
function renderMessages() {
  ...
  const canCall = !!state.currentChat && !state.currentChat.is_group;
  els.audioCallBtn.classList.toggle('hidden', !canCall);
  els.videoCallBtn.classList.toggle('hidden', !canCall);
  els.audioCallBtn.disabled = !canCall;
  els.videoCallBtn.disabled = !canCall;
  ...
  els.messages.innerHTML = messages.map((msg) => {
    ...
    const fileBlock = msg.type === 'file'
      ? `<a class="message-file" href="${API_BASE}/api/files/${msg.id}" target="_blank" rel="noreferrer">📎 <span>${escapeHtml(msg.file_name || 'file')}</span> <small>${escapeHtml(formatSize(msg.file_size))}</small></a>`
      : '';
    return `
...
function connectStream() {
  ...
  es.addEventListener('presence:update', (ev) => {
    ...
    if (state.currentChatMeta?.members) {
      state.currentChatMeta = {
        ...state.currentChatMeta,
        members: state.currentChatMeta.members.map((m) => (
          m.id === payload.userId ? { ...m, status: payload.status, online: payload.online, last_seen: payload.lastSeen } : m
        ))
      };
    }
...
async function ensureDirectPeerLoaded() {
  if (!state.currentChat || state.currentChat.is_group) return null;
  if (state.currentChat.peer_id) return state.currentChat.peer_id;
  const meta = await api(`/api/chats/${state.currentChat.id}/meta`);
  state.currentChatMeta = meta;
  const peer = meta.members.find((m) => m.id !== state.user.id);
  if (!peer) return null;
  state.currentChat.peer_id = peer.id;
...
async function startCall(kind) {
  if (!state.currentChat || state.currentChat.is_group) return;
  try {
    const peerId = await ensureDirectPeerLoaded();
    if (!peerId) {
      alert('Не удалось определить собеседника для звонка');
      return;
    }
    const { callId } = await api('/api/calls/start', { method: 'POST', body: JSON.stringify({ chatId: state.currentChat.id, targetUserId: peerId, kind }) });
    state.currentCall = { callId, kind, targetUserId: peerId, incoming: false };
    await prepareCallMedia(kind);
    await createPeer(true);
    showCallModal(`${kind === 'video' ? 'Видеозвонок' : 'Аудиозвонок'}`, 'Звоним…', { showEnd: true });
  } catch (err) {
    alert(err.message);
  }
}
