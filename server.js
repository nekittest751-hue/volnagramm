...
app.get('/api/files/:messageId', auth, async (req, res) => {
  try {
    const messageId = Number(req.params.messageId);
    const message = await get('SELECT id, chat_id, file_url, file_name FROM messages WHERE id = ? AND type = ?', [messageId, 'file']);
    if (!message) return res.status(404).json({ error: 'File not found' });

    const member = await ensureMember(req.user.id, message.chat_id);
    if (!member) return res.status(403).json({ error: 'Forbidden' });

    const filePath = path.join(uploadsDir, path.basename(message.file_url || ''));
    if (!fs.existsSync(filePath)) {
      return res.status(404).json({ error: 'File missing on server' });
    }
    res.download(filePath, message.file_name || path.basename(filePath));
  } catch (err) {
