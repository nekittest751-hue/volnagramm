...
app.post('/api/chats/direct', auth, async (req, res) => {
  ...
  const chat = await run(
    'INSERT INTO chats (title, is_group, created_by) VALUES (?, 0, ?)',
    [title, req.user.id]
  );

  await run(
    'INSERT INTO chat_members (chat_id, user_id) VALUES (?, ?)',
    [chat.lastID, req.user.id]
  );

  await run(
    'INSERT INTO chat_members (chat_id, user_id) VALUES (?, ?)',
    [chat.lastID, other.id]
  );

  const created = await get(
    'SELECT id, title, is_group FROM chats WHERE id = ?',
    [chat.lastID]
  );
...
app.post('/api/chats/group', auth, async (req, res) => {
  ...
  const chat = await run(
    'INSERT INTO chats (title, is_group, created_by) VALUES (?, 1, ?)',
    [title, req.user.id]
  );
...
app.post('/api/chats/:chatId/messages', auth, async (req, res) => {
  ...
  const result = await run(
    ...
    [chatId, req.user.id, String(text).trim()]
  );
...
initDb()
  .then(() => {
    app.listen(PORT, '0.0.0.0', () => {
      console.log(`Volnagramm running on port ${PORT}`);
    });
  })
  .catch((err) => {
